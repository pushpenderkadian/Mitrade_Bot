#Mi Trade Bot for opening a sell position and closing it Made by pushpender_3740
...
def sell(driver,name,quantity,lev):
    try:
    ...
    time.sleep(0.5)
    driver.find_element_by_xpath('//*[@id="hoverBtn"]/button[1]').click()
    for i in range(0,6):
    ...
    for i in all_vals:
        ...
        if txt==lev:
            i.click()
            break
    sell_b=driver.find_element_by_xpath('//*[@id="main-body"]/div/div[1]/div[2]/div/div/div[2]/div[3]/div/div[3]/div/div[2]/button')
    sell_b.location_once_scrolled_into_view
    sell_b.click()
...
def clos(driver):
    ...
    while True:
        try:
            ...
            break
    time.sleep(1)
    txt=driver.find_element_by_xpath('//*[@id="rc-tabs-0-panel-openPositions"]/ul/li').text
    if "Sell" in txt:
        driver.find_element_by_xpath('//*[@id="rc-tabs-0-panel-openPositions"]/ul/li').click()
    else:
        vls=driver.find_element_by_xpath('//*[@id="rc-tabs-0-panel-openPositions"]/ul')
        lst=vls.find_elements_by_tag_name('li')
        for i in lst:
            txt=i.text
            if "Sell" in txt:
                i.click()
...
def main():
    ...
    while True:
        ...
        if(ch=='1'):
            while True:
                try:
                    quantity=float(input("Enter the quantity between 0.01 to 30.00 of CHF/JPY to sell : "))
                    print(quantity)
                    if quantity>=0.01 and quantity<=30.00:
                        ...
                        sell(driver,nam,str(quantity),lev)
                        break
                    else:
                        hgchgcv
            ...
            while True:
                try:
                    quantity=float(input("Enter the quantity between 0.01 to 30.00 of GBP/USD to sell : "))
                    if quantity>=0.01 and quantity<=30.00:
                        nam="GBP/USD"
                        lev="30x"
                        page=driver.page_source
            ...
            while True:
                try:
                    quantity=int(input("Enter the quantity between 1 to 100 of SPX500 to sell : "))
